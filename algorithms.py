...
class AdverserialTraning(UpdatesAlgorithm):
    def __init__(self, gen_obj, dis_obj, step_rule, model,
                 ...
                 dis_iter=1, gradient_clip=[-0.01, 0.01],
                 **kwargs):
    ...
    def initialize(self):
        ...
        # prepare gradient clip
        if self.gradient_clip:
            updates = [(p, p.clip(self.gradient_clip[0], self.gradient_clip[1])
                        ) for p in self.model.dis_params]
            self.clip_weight = theano.function([], [], updates=updates)
    ...
    def process_batch(self, batch):
        ...
        dis_iter = 100 if self._n_call < 25 or self._n_call % 500 == 0 else \
            self.dis_iter
